using MultiDistances: probability_jaccard_distance, ProbabilityJaccard, precalculate
using MultiDistances: lempel_ziv_dict, GramDict
...
@testset "probability_jaccard_distance with strings" begin
    s1, s2 = "arne", "arnearne"
    d1, d2 = lempel_ziv_dict(s1, true), lempel_ziv_dict(s2, true)
    @test probability_jaccard_distance(d1, d2) == probability_jaccard_distance(s1, s2)
end

@testset "ProbabilityJaccard distance and precalculate" begin
    d = ProbabilityJaccard()
    s1, s2 = "arne", "arnearne"
    @test evaluate(d, s1, s2) == probability_jaccard_distance(s1, s2)
    @test isa(precalculate(d, s1), GramDict)

    d1, d2 = precalculate(d, s1), precalculate(d, s2)
